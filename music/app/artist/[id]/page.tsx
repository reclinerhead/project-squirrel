...
// Artist page (issue #116): hero, bio with read-more, top tracks, and the
// discography grid. Top tracks are fixture-curated today and play_history-
// ranked once Phase 2 starts collecting; the section renders the same either
// way. Similar-artists is a named follow-up, not a missing feature.
...
export default function ArtistPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const topTracks = getTopTracks(artist.id);
      ...
      {topTracks.length > 0 && (
        <section className="panel rounded-sm border border-line bg-panel">
          <div className="flex items-baseline justify-between gap-3 px-4 pb-2 pt-3">
            ...
            <span className="stamp text-[10px] text-inkfaint">fixture-ranked · play history later</span>
          </div>
