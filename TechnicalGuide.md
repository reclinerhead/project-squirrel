...
7. **Evaluate on the same ruler**: `yolo val` for both old and new weights against the *same* valid split before claiming improvement. Baseline: train-16, 0.936 mAP50 / 0.887 recall (all classes) on the 0705 valid split. *Class-set change caveat: a 2-class model can't be same-ruler'd against 3-class train-16 headline numbers — the first 2-class round establishes a fresh baseline on the `_2class` valid split (optionally re-val train-16 on that split once, as a bridge number).*
