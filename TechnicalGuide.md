...
- **Weather Post** (in `Dashboard.tsx`, next to the Field Journal): current conditions + a hand-rolled SVG trend chart — observed temps trail solid to the left of a dashed "now" line (from `weather/history`), the forecast extends dashed to the right (from `weather/forecast`, bridged at the last observed point so the lines meet), wind on its own scale underneath. 24h back + 48h forward puts "now" at the 1/3 mark. Its own mqtt.js client (same shape as the Field Journal's — panels stay self-contained; Mosquitto shrugs at a second WebSocket) subscribes to the three retained topics, so a fresh tab paints weather instantly with zero HTTP. A report older than 30 minutes flips the header lamp to a stale warning rather than presenting old weather as now. The panel reserves its full footprint before data arrives (the no-layout-shift rule), and its empty states distinguish bus-unreachable from no-weather-service-publishing. Pure parsing/shaping (`parseCurrent`, `parsePoints`, `trendSeries`, `tempRange`, `windCeil`, `linePath`, `compass`) lives in `lib/weather.ts` with Vitest coverage; temp axis pads to a 16°F minimum span so a flat day reads flat, wind axis floors at 10 mph so calm days hug the baseline.
